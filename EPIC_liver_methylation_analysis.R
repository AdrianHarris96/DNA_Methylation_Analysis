...
if (file.exists(paste(output_dir, "rgSet.RDS", sep=""))) {
  ...
  cat('Generate rgSet\n')
  rgSet <- read.metharray.exp(base=dirEPIC, target=phenoEPIC)
  ?read.metharray.exp()
...
colnames(design) <- c("DD_HI_L1","DD_HI_L2","DD_LI_L1","DD_LI_L2")
...
write.csv(log_df, file = paste(output_dir, "EPIC_liver_log.csv", sep=""), row.names = FALSE)
