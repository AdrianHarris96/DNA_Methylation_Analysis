...
pheno_df <- pheno_df[(pheno_df$array_type == '450K'),]
...
if (file.exists(paste(output_dir, "rgSet.RDS", sep=""))) {
  ...
  cat('Generate rgSet\n')
  rgSet <- read.metharray.exp(base=dir450k, target=pheno_df, force=TRUE)
...
if (file.exists(paste(output_dir, "beta_values.csv", sep=""))) {
  ...
  barplot((subset(detP_df, array_type == '450K'))$p_value_mean, col=pal[factor(detP_df$time)], names.arg=(subset(detP_df, array_type == '450K'))$sample_id, las=2, cex.names=0.4, cex.axis=0.5, space=0.5, ylab="Mean detection p-values", main='450K Array')
...
for (outcome in eGFR_List) {
  ...
  ann450kSub <- ann450k[match(rownames(m_values_condition),ann450k$Name),
  ...
  final_title <- paste(outcome, "_450k_kidney_log.csv", sep="")
