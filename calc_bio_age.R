...
#Drop Horvath age if it exists in the df
if ("Horvath" %in% colnames(pheno_df)) {
  # if it exists, drop it
  pheno_df <- subset(pheno_df, select = -Horvath)
}
...
#Dropping the single, unpaired sample
pheno_df1 <- pheno_df1[!(pheno_df1$sample_name == 'V037 L1'),]

#Formatting prior to the merge by creating pair column
...
#Calculating difference across timepoints
final_df$diff <- 'na'
for (row in 1:nrow(final_df)) {
  diff <- final_df[row, 'Horvath.y'] - final_df[row, 'Horvath.x']
  final_df[row, 'diff'] <- diff
