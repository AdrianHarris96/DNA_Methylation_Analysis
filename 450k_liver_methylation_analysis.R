...
if (file.exists(paste(output_dir, "rgSet.RDS", sep=""))) {
  ...
  cat('Generate rgSet\n')
  rgSet <- read.metharray.exp(base=dir450k, target=pheno450k)
  ?read.metharray.exp()
...
DMPs1_sig <- DMPs1[(DMPs1$adj.P.Val < 0.05 & DMPs1_sig$deltaBeta > 0.2),]
print(dim(DMPs1_sig))
...
DMPs2_sig <- DMPs2[(DMPs2$adj.P.Val < 0.05 & DMPs2_sig$deltaBeta > 0.2),]
print(dim(DMPs2_sig))
...
DMPs3_sig <- DMPs3[(DMPs3$adj.P.Val < 0.05 & DMPs3_sig$deltaBeta > 0.2),]
print(dim(DMPs3_sig))
...
DMPs4_sig <- DMPs4[(DMPs4$adj.P.Val < 0.05 & DMPs4_sig$deltaBeta > 0.2),]
print(dim(DMPs4_sig))
...
DMPs5_sig <- DMPs5[(DMPs5$adj.P.Val < 0.05 & DMPs5_sig$deltaBeta > 0.2),]
print(dim(DMPs5_sig))
...
DMPs6_sig <- DMPs6[(DMPs6$adj.P.Val < 0.05 & DMPs6_sig$deltaBeta > 0.2),]
print(dim(DMPs6_sig))
...
DMPs7_sig <- DMPs7[(DMPs7$adj.P.Val < 0.05 & DMPs7_sig$deltaBeta > 0.2),]
print(dim(DMPs7_sig))
...
DMPs8_sig <- DMPs8[(DMPs8$adj.P.Val < 0.05 & DMPs8_sig$deltaBeta > 0.2),]
print(dim(DMPs8_sig))
...
DMPs9_sig <- DMPs9[(DMPs9$adj.P.Val < 0.05 & DMPs9_sig$deltaBeta > 0.2),]
print(dim(DMPs9_sig))
...
write.csv(log_df, file = paste(output_dir, "450k_liver_log.csv", sep=""), row.names = FALSE)
...
generate_man <- function(DMPs, comp) {
  ...
  jpeg(paste(output_dir, output, sep=""), quality = 100)
  manhattan(DMPs, chr="chr", bp="pos",, p="deltaBeta", snp="Islands_Name", col=col, suggestiveline=(0.2), main=title)
  dev.off()
