...
colnames(beta_values_filtered) <- substring(colnames(beta_values_filtered), 2)
print(colnames(beta_values_filtered))
...
generate_dendro <- function(beta, pheno, timepoint){
  ...
  for (row in 1:nrow(final_beta)) {
    ...
    final_beta[row, 'sample_id_CIT'] <- cit
...
for (outcome in eGFR_List) {
  ...
  final_title <- paste(outcome, "_combined_kidney_log.csv", sep="")
