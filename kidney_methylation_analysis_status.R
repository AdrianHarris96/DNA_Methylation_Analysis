...
eGFR_List <- c('eGFR_1month-eGFR_12month', 'eGFR_12month-eGFR_24month')
for (outcome in eGFR_List) {
  ...
  deltaBeta_df <- get_deltaBeta("K1_High_High", "K2_High_High", pheno)
  sample_num <- nrow(subset(pheno, (condition == 'K1_High_High' | condition == 'K2_High_High')))
  DMPs1 <- merge(DMPs1, deltaBeta_df, by = 'Name')
  output <- paste(outcome, "K1_High_High-K2_High_High_DMPs.csv", sep="-")
  write.csv(DMPs1, file = paste(output_dir, output, sep=""), row.names = FALSE) 
  DMPs1_sig <- DMPs1[(DMPs1$adj.P.Val < 0.05),]
  print(dim(DMPs1_sig))
  output <- paste(outcome, "K1_High_High-K2_High_High_DMPs_sig.csv", sep = "-")
  write.csv(DMPs1_sig, file = paste(output_dir, output, sep=""), row.names = FALSE)
  log_df[nrow(log_df) + 1,] <- c("K1_High_High-K2_High_High", sample_num, nrow(DMPs1_sig))
  ...
  if (nrow(DMPs2_sig) == 0) {
    ...
    myAnnotation <- cpg.annotate(datatype = "array", object = as.matrix(m_values_condition), what = "M",
                                 analysis.type = "differential", design = design,
                                 contrasts = TRUE, cont.matrix = contMatrix,
                                 coef = "K1_Low_Low - K2_Low_Low", arraytype = "EPIC")
    DMRs <- dmrcate(myAnnotation, lambda=1000, C=2)
    if (is.null(DMRs)) {
      print('Probes spaced beyond 1000 nucleotides')
    } else {
      results.ranges <- extractRanges(DMRs, genome = "hg19")
      output <- paste(outcome, "K1_Low_Low-K2_Low_Low_DMRs.csv", sep="-")
      write.csv(results.ranges, file=paste(output_dir, output, sep=""), row.names=FALSE)
    }
  }
  ...
  if (nrow(DMPs3_sig) == 0) {
    ...
    myAnnotation <- cpg.annotate(datatype = "array", object = as.matrix(m_values_condition), what = "M",
                                 analysis.type = "differential", design = design,
                                 contrasts = TRUE, cont.matrix = contMatrix,
                                 coef = "K1_Low_Low - K1_High_High", arraytype = "EPIC")
    DMRs <- dmrcate(myAnnotation, lambda=1000, C=2)
    if (is.null(DMRs)) {
      print('Probes spaced beyond 1000 nucleotides')
    } else {
